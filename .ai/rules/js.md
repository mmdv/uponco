...
## SW offline cache must exclude Inertia partial reloads
The NetworkFirst offline route (/dashboard, /appointments) must NOT cache Inertia partial reloads (requests with header X-Inertia-Partial-Data, e.g. workingHoursWindow/availableSlots). Their responses are incomplete pages; serving one to Inertia's setPage crashes at hrefToUrl(pageResponse.url) with "undefined is not an object (evaluating 'e.toString')" when the body has no url (also caused by caching opaque status-0 responses). Eligibility lives in resources/js/lib/offline-page-cache.ts (isCacheablePageRequest), and CacheableResponsePlugin must allow status [200] only, never [0,200].
